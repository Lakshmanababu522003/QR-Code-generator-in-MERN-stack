...
const Register = () => {
    ...
    return (
        ...
        onSubmit={async(e)=>{
            ...
            try {
                if(!username.trim()||!email.trim()){
                    ...
                    const {data} = await axios.post('http://localhost:4000/api/api/register',{username,email},config);
        ...
        >
            ...
            <input className="form-control w-50 my-2" type="text" placeholder="Enter your name" aria-label="default input example" value={username}
            onChange={(e)=>setUsername(e.target.value)}
            />
            <input className="form-control w-50 my-2" type="text" placeholder="Enter your email" aria-label="default input example" value={email}
